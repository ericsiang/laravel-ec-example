...
<script>
    ...
    function on_plus(cart_id){
        var new_quantity=$('#qty_'+cart_id+'').val();
        var new_quantity=parseInt(new_quantity)+1;
        $.ajax({
            url:'/update_cart',
            type:'post',
            async:false,
    ...
    <div class="container relative">
            ...
            <table class="shop_table cart table">
                ...
                <tbody>
                    @if (count($OrderCarts)==0)
                        <tr class="cart_item" ><td style="color:#FF0000;">尚無商品</td></tr>
                    @endif

                    @foreach ($OrderCarts as $OrderCart)
                        @php
                            $product=\App\Product::WHERE('p_id',$OrderCart->p_id)->get();
                            $total+=$product[0]->price*$OrderCart->quantity;
                        @endphp              

                        <tr class="cart_item">
                            <td class="product-thumbnail">
                                <a href="/SingleProduct/{{ $OrderCart->p_id }}">
                                    <img src="{{ asset('storage/'.$product[0]->img) }}" alt="">
                                </a>
                            </td>
                            <td class="product-name">
                                <a href="/SingleProduct/{{ $OrderCart->p_id }}">{{ $product[0]->name }}</a>
                            </td>
                            <td class="product-price">
                                <span class="amount">${{ $product[0]->price }}</span>
                            </td>
                            <td class="product-quantity">
                                
                                <div class="quantity buttons_added">
                                    <input type="button" value="-" class="minus" onClick="on_min({{ $OrderCart->cart_id }});">
                                    <input type="number" id="qty_{{ $OrderCart->cart_id }}" name="order_cart[{{ $OrderCart->cart_id }}]" step="1" min="0"  title="Qty" class="input-text qty text" value="{{ $OrderCart->quantity }}">
                                    <input type="button" value="+" class="plus" onClick="on_plus({{ $OrderCart->cart_id }});">
                                </div>
                                
                            </td>
                            <td class="product-subtotal">
                                <span class="amount">${{ $product[0]->price*$OrderCart->quantity }}</span>
                            </td>
                            <td class="product-remove">
                                <a href="javascript:void(0);" class="remove" title="Remove this item" onClick="on_delete({{ $OrderCart->cart_id }});">
                                    <i class="ui-close"></i>
                                </a>
                            </td>
                        </tr>
        ...
        <div class="row mb-30">
            ...
            <div class="col-lg-7">
                <div class="actions">
                    <!--
                    <input  name="update_cart" onClick="$('#UpdateCartForm').submit();" value="Update Cart" class="btn btn-md btn-dark btn-button">-->
                    <div class="wc-proceed-to-checkout">
                        <a href="/checkout" class="btn btn-md btn-color btn-button"><span>proceed to
                                checkout</span></a>
                    </div>
                </div>
            </div>
